...
class Uc_Irvine_Message_Dataset():
	def __init__(self,args):
		args.uc_irc_args = utils.Namespace(args.uc_irc_args)

		edges_file = os.path.join(args.uc_irc_args.folder, args.uc_irc_args.edges_file)  
		# tar_archive = tarfile.open(tar_file, 'r:bz2')

		self.edges = self.load_edges(args,edges_file)

	def load_edges(self,args,edges_file):
		# data = utils.load_data_from_tar(args.uc_irc_args.edges_file, 
		# 							tar_archive, 
		# 							starting_line=2,
		# 							sep=' ')
		f = open(edges_file, 'r')
		lines = f.readlines()
		f.close()
		data = torch.DoubleTensor([[float(r) for r in row.split(' ')] for row in lines[2:]])
