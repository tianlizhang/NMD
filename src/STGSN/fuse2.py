    ...
    def forward(self, sup_list, feat_list, gbl_sup, gbl_feat, num_nodes):
        ...
        for l in range(self.num_struc_layers):
            ...
            for i in range(win_size):
                ...
                if l == 0:
                    cites_output = self.cites_enc[l](ind_input, ind_sup)
                else:
                    cites_output = self.cites_enc[l](cites_input_list[i], ind_sup)
        ...
        ind_emb_list, cites_emb_list = [], []
        for out, out_cites in zip(ind_output_list, cites_output_list):
            ind_fuse = self.mlp(self.fusion(out, out_cites))
            cites_fuse = self.mlp(self.fusion(out_cites, out))
            
            ind_emb_list.append(ind_fuse)
            cites_emb_list.append(cites_fuse)
        
        cites_list = cites_loader.build_cites(sup_list)
        node_loss = self.calc_cites(cites_emb_list, cites_list)
        # ==========
        # Temporal encoder
