...
class dyngraph2vec_Enc(nn.Module):
    ...
    def forward(self, adj_list):
        ...
        # ==========
        # Fusion layer
        temp_input = self.mlp(self.fusion(temp_input_tnr, cites_input_tnr))
        cites_input_tnr = self.mlp(self.fusion(cites_input_tnr, temp_input_tnr))
        ...
        # ==========
        # Temporal encoder
        temp_input = torch.reshape(temp_input, (win_size, int(num_nodes), self.temp_dims[0]))
        temp_output = None
        for l in range(self.num_temp_layers):
