...
class DDNE_Enc(nn.Module):
    ...
    def forward(self, adj_list):
        ...
        for l in range(self.num_enc_layers):
            ...
            # ==========
            for_cites_output, _ = self.for_RNN_layer_list[l](for_cites_input)
            for_cites_input = for_cites_output
            # ==========
            rev_cites_output, _ = self.rev_RNN_layer_list[l](rev_cites_input)
            rev_cites_input = rev_cites_output
        # ==========
        # Fusion layer
        RNN_out_list = [self.mlp(self.fusion(torch.hstack([for_out, rev_out]), torch.hstack([for_cite, rev_cite]))) \
