...
class Cross_Entropy(torch.nn.Module):
    ...
    def logsumexp(self,logits):
        m,_ = torch.max(logits,dim=1) # values, indices
        m = m.view(-1,1)
    ...
    def forward(self,logits,labels):
        ...
        alpha = self.weights(labels)[labels].view(-1,1) # self.weights(labels): [0.1, 0.9]; alpha: [n, 1](0.9 or 0.1)
        loss = alpha * (- logits.gather(-1,labels) + self.logsumexp(logits)) # logits.gather(-1,labels): [n, 1]
        return loss.mean()
