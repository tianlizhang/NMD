...
class GCN_GAN(nn.Module):
    ...
    def __init__(self, args, num_nodes, dropout_rate):
        ...
        # ====================
        self.struc_dims = [args.feats_per_node, 8] # Layer configuration of structural encoder
        self.temp_dims = [num_nodes*self.struc_dims[-1], 8] # Layer configuration of temporal encoder
        self.dec_dims = [self.temp_dims[-1], num_nodes*num_nodes] # Layer configuration of decoder
    ...
    def forward(self, sup_list, noise_list=None, mask_list=None):
        '''
        ...
        #     sup_tnr = get_gnn_sup(A_list[t])
        #     sup_list.append(sup_tnr)
        #     # =========
        #     # Generate random noise
        #     noise_feat = torch.rand([num_nodes, self.struc_dims[0]])
        #     noise_feat_tnr = torch.FloatTensor(noise_feat).to(A_list[0].device)
...
class GCN_GAN_Enc(nn.Module):
    ...
    def __init__(self, struc_dims, temp_dims, dropout_rate):
        ...
        # ==========
        # Fusion Modules
        self.fusion_mode = 'cat'
        self.pred_year = 1
        hidden_channels = self.enc_dims[-1]
        self.mlp_edge = nn.Linear(2*hidden_channels, hidden_channels)
...
class DiscNet(nn.Module):
    ...
    def __init__(self, args, num_nodes, dropout_rate):
        ...
        # ====================
        self.disc_dims = [num_nodes*num_nodes, 64, 16, 1] # Layer configuration of discriminator
        self.dropout_rate = dropout_rate # Dropout rate
        # ==========
        self.num_disc_layers = len(self.disc_dims)-1 # Number of FC layers
