...
class gcn_gan_loss(torch.nn.Module):
    """docstring for Cross_Entropy"""
    def __init__(self, args):
        super().__init__()
        self.alpha = args.alpha
    
    def get_gen_loss(self, adj_est, gnd, disc_fake):
        # ====================
        loss = -torch.mean(disc_fake) # Loss of GAN
        loss += self.alpha*torch.norm((adj_est - gnd), p='fro')**2 # MSE loss of error minimization

        return loss

    def get_disc_loss(self, disc_real, disc_fake):
        # ====================
        loss = torch.mean(disc_fake) - torch.mean(disc_real)
        #loss = torch.mean(torch.log(disc_fake)) - torch.mean(torch.log(disc_real))
